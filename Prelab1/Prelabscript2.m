...
mu=0;
linear_trend_factor=0.1*A; %Størrelse på drift
...
%også at gjennomsnittet ikke ser ut til å være nøyaktig null, både pga.
%noise og pga. drift
%%%
...
title('Histogram av støyet');
xlabel('Volt, V [V]');
ylabel('Absolutt frekvens');
histogram(gaussian_noise) %Histogram kun av støy

%Normalisering av støy histgoramet
...
title('Normalisert histogram av støyet, med normalfordeling');
xlabel('Volt, V [V]');
ylabel('Relativ frekvens');
legend('Data', 'Normal distribution');


%Ta ut lineær trend - bruker innebyd Matlab funksjon
figure(5)
trend=detrend(Data);
hold off
plot(x, trend);
